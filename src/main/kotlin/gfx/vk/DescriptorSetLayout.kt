...
abstract class DescriptorSetLayout(private val device: Device) {
    ...
    fun cleanup() {
        ...
        VK13.vkDestroyDescriptorSetLayout(device.vkDevice, vkDescriptorLayout, null)
    }
    ...
    open class SimpleDescriptorSetLayout(device: Device, descriptorType: Int, binding: Int, stage: Int) :
        ...
        init {
            MemoryStack.stackPush().use { stack ->
                ...
                vkCheck(
                    VK13.vkCreateDescriptorSetLayout(device.vkDevice, layoutInfo, null, pSetLayout),
    ...
    class SamplerDescriptorSetLayout(device: Device, binding: Int, stage: Int) :
        SimpleDescriptorSetLayout(device, VK13.VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER, binding, stage)
    class UniformDescriptorSetLayout(device: Device, binding: Int, stage: Int) :
        SimpleDescriptorSetLayout(device, VK13.VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER, binding, stage)
    class DynUniformDescriptorSetLayout(device: Device, binding: Int, stage: Int) :
        SimpleDescriptorSetLayout(device, VK13.VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER_DYNAMIC, binding, stage)
